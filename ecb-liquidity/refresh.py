...
def collect(source_dir=None):
    ...
    current_sources=[name for name in ('api',str(today.year)) if name in successful]
    current_latest=max((max(successful[name][0]) for name in current_sources if successful[name][0]),default=None)
    old_latest=max(oldvalues,default=None)
    latest_data_check_succeeded=bool(current_sources and current_latest and (not old_latest or current_latest>=old_latest))
    ...
    return 0 if fresh and complete and latest_data_check_succeeded else 2
